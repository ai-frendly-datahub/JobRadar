...
def generate_report(
    *,
    ...
    if quality_report:
        _inject_job_quality_panel(report_path, quality_report)
    return report_path
...
def generate_index_html(
    ...
    radar_name = "Job Radar"
    return _core_generate_index_html(report_dir, radar_name)


def _inject_job_quality_panel(report_path: Path, quality_report: Mapping[str, Any]) -> None:
    if not report_path.exists():
        return
    html = report_path.read_text(encoding="utf-8")
    panel = _render_job_quality_panel(quality_report)
    marker = "</body>"
    if marker in html:
...
def _render_job_quality_panel(quality_report: Mapping[str, Any]) -> str:
    ...
    cards = [
        ("Job signals", summary.get("job_signal_event_count", 0)),
        ("Official postings", summary.get("official_job_posting_events", 0)),
        ("Job-board posts", summary.get("job_board_posting_events", 0)),
        ("Skill demand", summary.get("skill_demand_events", 0)),
        ("Salary signals", summary.get("salary_signal_events", 0)),
    ...
    cards_html = "\n".join(
        "<div class=\"job-quality-card\">"
        f"<span>{escape(label)}</span><strong>{escape(str(value))}</strong>"
        "</div>"
        for label, value in cards
    )
    return f"""
<section id="job-quality" class="job-quality-panel">
  <style>
    .job-quality-panel {{ margin: 32px auto; max-width: 1180px; padding: 24px; border: 1px solid #d8dee4; border-radius: 8px; background: #fff; color: #24292f; }}
    .job-quality-panel h2 {{ margin: 0 0 8px; font-size: 1.35rem; }}
    .job-quality-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; margin: 16px 0 22px; }}
    .job-quality-card {{ border: 1px solid #d8dee4; border-radius: 8px; padding: 10px 12px; background: #f6f8fa; }}
    .job-quality-card span {{ display: block; font-size: .82rem; color: #57606a; }}
    .job-quality-card strong {{ display: block; margin-top: 4px; font-size: 1.2rem; }}
    .job-quality-table {{ width: 100%; border-collapse: collapse; margin-top: 12px; font-size: .9rem; }}
    .job-quality-table th, .job-quality-table td {{ border-top: 1px solid #d8dee4; padding: 8px; text-align: left; vertical-align: top; }}
    .job-quality-review {{ margin: 12px 0 0; padding-left: 18px; }}
  </style>
  <h2>Job Quality</h2>
  <p>Operational hiring evidence is separated from broad HR news and community proxy signals.</p>
  <div class="job-quality-grid">
    {cards_html}
  </div>
...
def _render_quality_events(events: list[Mapping[str, Any]]) -> str:
    if not events:
        return "<p>No job quality events were observed in this report window.</p>"
    rows = []
    ...
    return (
        ...
        "<table class=\"job-quality-table\"><thead><tr>"
...
def _render_quality_review(review_items: list[Mapping[str, Any]]) -> str:
    ...
    return "<h3>Daily Review</h3><ul class=\"job-quality-review\">" + "\n".join(items) + "</ul>"
